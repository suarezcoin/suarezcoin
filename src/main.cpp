...
unsigned int nTargetSpacing = 60 * 1; // 60 seconds
unsigned int nStakeMinAge = 60 * 60 * 24; // 24 hours
unsigned int nStakeMaxAge = 60 * 60 * 24 * 48; //48 days
...
const string strMessageMagic = "SuarezCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if (pindexBest->nHeight+1 <= 2601)
    {
      nSubsidy = 500 * COIN;
      return nSubsidy + nFees;
    }
    else if (pindexBest->nHeight+1 <= 22601)
    {
      nSubsidy = 300 * COIN;
      return nSubsidy + nFees;
    }
    else if (pindexBest->nHeight+1 <= 33101)
    {
      nSubsidy = 200 * COIN;
      return nSubsidy + nFees;
    }
    else if (pindexBest->nHeight+1 <= 34101)
    {
      nSubsidy = 500 * COIN;
      return nSubsidy + nFees;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SuarezCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "One More Bite // Launching 020714";
        CTransaction txNew;
        txNew.nTime = 1404170487;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1404170487;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2157267;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xc411c3e02487d6a186a788139ad147d361a50d1700bf3d322c7b19c227022ed4"));
        block.print();
